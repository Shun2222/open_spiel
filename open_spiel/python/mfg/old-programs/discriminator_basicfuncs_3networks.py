...
class Discriminator(nn.Module):
    def __init__(self, distance_input, n_agent, ob_shape, ac_shape, state_only, device, discount=0.99, hidden_size=128, l2_loss_ratio=0.01):
        ...
        self.distance_net = nn.Sequential(
            nn.Linear(distance_input, hidden_size),
            nn.ReLU(),
        ...
        self.ac_net = nn.Sequential(
            nn.Linear(ac_shape, hidden_size),
            nn.ReLU(),
            nn.Linear(hidden_size, 1)
        ).to(self._device)
        ...
        self.value_net = nn.Sequential(
            nn.Linear(ob_shape+n_agent, hidden_size),
            nn.ReLU(),
            nn.Linear(hidden_size, 1)
        ).to(self._device)
...
#         )
    ...
    def save(self, filename=""):
        ...
        fname = osp.join(logger.get_dir(), "disc_ac"+filename+".pth")
        torch.save(self.ac_net.state_dict(), fname)
    ...
    def load(self, distance_path, mu_path, ac_path, reward_path, value_path, use_eval=False):
        self.distance_net.load_state_dict(torch.load(distance_path))
        self.mu_net.load_state_dict(torch.load(mu_path))
        self.ac_net.load_state_dict(torch.load(ac_path))
    ...
    def print_weights(self, only_rew=True):
        if only_rew:
            ...
            networks = [self.distance_net, self.mu_net, self.ac_net, self.reward_net]
