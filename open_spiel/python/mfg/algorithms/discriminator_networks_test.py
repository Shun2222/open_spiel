...
class Discriminator(nn.Module):
    def __init__(self, input_shapes, obs_shape, labels, device, discount=0.99, hidden_size=128, l2_loss_ratio=0.01, num_hidden=1, ppo_value_net=None):
        ...
        assert 2==len(input_shapes), 'num networks error'
        self.n_networks = 2 
         
        self.network1 = nn.Sequential(
                            nn.Linear(input_shapes[0], hidden_size),
                            nn.ReLU(),
                            nn.Linear(hidden_size, 1)
                        ).to(self._device)
        self.network2 = nn.Sequential(
                            nn.Linear(input_shapes[1], hidden_size),
                            nn.ReLU(),
                            nn.Linear(hidden_size, 1)
                        ).to(self._device)
    ...
    def forward(self, dxy, mu, obs, obs_next, path_probs):
        #rew_input = obs if self.state_only else torch.cat([obs, acs], dim=1)
        out1 = self.network1(dxy.to(torch.float32))
        out2  = self.network2(mu.to(torch.float32))
    ...
    def calculate_loss(self, dxy, mu, obs, obs_next, path_probs, labels):
        log_q_tau, log_p_tau, log_pq, discrim_output = self.forward(dxy, mu, obs, obs_next, path_probs)
    ...
    def train(self, dxy, mu, optimizer, obs, obs_next, path_probs, labels):
        optimizer.zero_grad()
        loss = self.calculate_loss(dxy, mu, obs, obs_next, path_probs, labels)
        loss.backward()
        optimizer.step()
        return loss.item()
    ...
    def get_reward(self, inputs, obs, obs_next, path_probs, discrim_score=False, only_rew=True, weighted_rew=False):
        with torch.no_grad():
            if discrim_score:
                ...
                outputs = [out1, out2]
                rew_inputs = torch.cat((out1, out2), dim=1)
    ...
    def save(self, filename=""):
        ...
        torch.save(self.network1.state_dict(), fname)

        fname = osp.join(logger.get_dir(), "disc_"+f"{self.labels[1]}"+filename+".pth")
        torch.save(self.network2.state_dict(), fname)
    ...
    def load(self, path, filename, use_eval=False):
        ...
        self.network1.load_state_dict(torch.load(fname))
        fname = osp.join(path, "disc_"+f"{self.labels[1]}"+filename+".pth")
        self.network2.load_state_dict(torch.load(fname))
    ...
    def load_with_path(self, pathes, reward_path, value_path, use_eval=False):
        self.network1.load_state_dict(torch.load(pathes[0]))
        self.network2.load_state_dict(torch.load(pathes[1]))
    ...
    def print_weights(self, only_rew=True):
        if only_rew:
            ...
            networks = [self.network1, self.network2] + [self.reward_net]
