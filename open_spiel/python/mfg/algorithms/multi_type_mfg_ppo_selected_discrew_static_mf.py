...
def parse_args():
    ...
    parser.add_argument("--seed", type=int, default=2, help="set a random seed")
    ...
    for rate in combinations:
        ...
        for i in range(num_agent):
            if single:
                ...
                all_net_input.append(net_inputs)
            else:
        ...
        if args.save_disc_reward:
            for target_i in [0]:
                ...
                rewards1 = np.zeros((horizon, size, size, nacs))
                num_nets = 2
                    ...
                    for x in range(size):
                        for y in range(size):
                            for a in range(nacs):
                                ...
                                reward0, outputs0 = discriminators[target_i][0].get_reward(
                                    rew_input,
                                    discrim_score=False,
                                    only_rew=False,
                                    weighted_rew=True) # For competitive tasks, log(D) - log(1-D) empirically works better (discrim_score=True)

                                reward1, outputs1 = discriminators[i][0].get_reward(
                                    rew_input,
                                    discrim_score=False,
                                    only_rew=False,
                                    weighted_rew=True) # For competitive tasks, log(D) - log(1-D) empirically works better (discrim_score=True)

                                rewards0[t, y, x, a] = reward0
                                rewards1[t, y, x, a] = reward1
