...
class MultiTypeAIRL(object):
    def __init__(self, game, envs, merge_dist, conv_dist, device, experts, ppo_policies):
        ...
        self._discriminator = [Discriminator(self._nobs, self._nacs, False, device) for _ in range(self._num_agent)]
        ...
        while(t_step < total_step):
            for neps in range(num_episodes): 
                ...
                for idx, rout in enumerate(rollouts):
                    ...
                    if buffer[idx]:
                        buffer[idx].update(mh_obs, mh_actions, mh_obs_next, all_obs, mh_values)
                    else:
                        buffer[idx] = Dset(mh_obs, mh_actions, mh_obs_next, all_obs, mh_values, 
                                      randomize=True, num_agents=1, nobs_flag=True)

                    e_obs, e_actions, e_nobs, e_all_obs, _ = self._experts[idx].get_next_batch(batch_step)
                    g_obs, g_actions, g_nobs, g_all_obs, _ = buffer[idx].get_next_batch(batch_step)
                    ...
                    for i in range(len(e_obs[0])):
                        e_log_prob.append(self._generator[idx].get_log_action_prob(
                            torch.from_numpy(np.array([e_obs[0][i][:self._nobs]])).to(torch.float32).to(self._device), 
                        ...
                        g_log_prob.append(self._generator[idx].get_log_action_prob(
                            torch.from_numpy(np.array([g_obs[0][i][:self._nobs]])).to(torch.float32).to(self._device), 
                    ...
                    total_loss = self._discriminator[idx].train(
                        ...
                        torch.from_numpy(d_obs).to(torch.float32).to(self._device),
