...
class MultiTypeMFGPPO(object):
    def __init__(self, game, env, merge_dist, conv_dist, discriminator, device, player_id=0, expert_policy=None, is_nets=True, is_divided=True, net_input=None, rew_index=-1, rate=[1.0, 1.0]):
        self._device = device
    ...
    def rollout(self, env, nsteps):
        ...
        while step!=nsteps:
            ...
            while not time_step.last():
                ...
                if self._is_nets:
                    if self._is_divided:
                        reward, weighted_reward, outputs = self._discriminator.get_reward_weighted(
                            inputs,
                            rate=self._rate) # For competitive tasks, log(D) - log(1-D) empirically works better (discrim_score=True)
                    else:
...
def parse_args():
    ...
    parser.add_argument('--logdir', type=str, default="/mnt/shunsuke/result/0726/multi_maze2_dxy_mu_weigted-mul", help="logdir")
...
if __name__ == "__main__":
    ...
    for rate in rates:
        ...
        for i in range(len(rate)):
            path+=f'-{rate[i]}'
        logger.configure(path, format_strs=['stdout', 'log', 'json'])
        logger.record_tabular(f"rate", rate)
        logger.dump_tabular()
        ...
        mfgppo = [MultiTypeMFGPPO(game, envs[i], merge_dist, conv_dist, discriminators[i], device, player_id=i, is_nets=is_nets, is_divided=is_divided, net_input=net_input, rew_index=rew_index, rate=rate) for i in range(num_agent)]
