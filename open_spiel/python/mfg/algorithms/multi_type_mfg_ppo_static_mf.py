...
class MultiTypeMFGPPO(object):
    ...
    def rollout(self, env, nsteps):
        ...
        ret = np.array(ret)
        assert step==nsteps
        return info_state, actions, logprobs, true_rewards, true_rewards, dones, values, entropies,t_actions,t_logprobs, all_mu, ret
...
def parse_args():
    ...
    parser.add_argument("--num_seed", type=int, default=1, help="set a random seed")
    ...
    parser.add_argument("--path", type=str, default="/mnt/shunsuke/result/icaart/conventional_method/multi_maze2_airl_deltaxy_1000trajs/seed-42", help="file path")
    parser.add_argument('--logdir', type=str, default="/mnt/shunsuke/result/icaart/eval/NIG", help="logdir")

    parser.add_argument("--rew_index", type=int, default=-1, help="-1 is reward, 0 or more are output")
    parser.add_argument("--update_eps", type=str, default=r"500_5", help="file path")
...
if __name__ == "__main__":
    ...
    for seed in seeds:
        ...
        for i in range(num_agent):
            ...
            actor_path = osp.join(args.path, "actor"+update_eps_info + f'-{i}.pth')
            actor_model.load_state_dict(torch.load(actor_path))
            actor_model.eval()

            critic_path = osp.join(args.path, "critic"+update_eps_info + f'-{i}.pth')
        ...
        envs = []
        for i in range(num_agent):
            envs.append(rl_environment.Environment(game, mfg_distribution=gen_merge_dist, mfg_population=i))
            envs[-1].seed(seed)
        
        conv_dist = convert_distrib(envs, gen_merge_dist)
        ...
        discriminators = [None for _ in range(num_agent)]
        mfgppo = [MultiTypeMFGPPO(game, envs[i], gen_merge_dist, conv_dist, discriminators[i], device, player_id=i, is_nets=is_nets, net_input=net_input, rew_index=rew_index) for i in range(num_agent)]
        ...
        for niter in tqdm(range(args.num_iterations)):
            ...
            for i in range(num_agent):
                ...
                nashc_ppo = mfgppo[i].update_iter(game, envs[i], None, None, nashc=True, population=i)
