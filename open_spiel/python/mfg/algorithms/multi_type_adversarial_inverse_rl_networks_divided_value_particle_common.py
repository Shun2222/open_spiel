...
from open_spiel.python.mfg.algorithms.discriminator_networks_divided_value_shared_reward import SharedReward, Discriminator_2nets_SharedReward
from games.predator_prey import goal_distance, divide_obs


class MultiTypeAIRL(object):
    def __init__(self, game, envs, merge_dist, conv_dist, device, experts, ppo_policies, disc_type='s_mu_a', disc_num_hidden=1, use_ppo_value=False, skip_train=[False, False, False], skip_agents=[None, None, None], common_index=[1, 1, 1]):
        self._game = game
        self._envs = envs
        self._device = device
        ...
        if use_ppo_value:
            if len(inputs)==2:
                ...
                for i in range(self._num_agent):
                    if common_index==1:
                        discriminator = Discriminator_2nets_SharedReward(self._shared_reward, inputs, obs_xym_size, labels, device, num_hidden=disc_num_hidden, ppo_value_net=self._generator[i]._eps_agent.critic) for i in range(self._num_agent)
                    else:
                        discriminator = Discriminator_2nets(inputs, obs_xym_size, labels, device, num_hidden=disc_num_hidden, ppo_value_net=self._generator[i]._eps_agent.critic) for i in range(self._num_agent)
