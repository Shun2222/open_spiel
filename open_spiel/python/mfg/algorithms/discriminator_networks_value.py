...
class Discriminator(nn.Module):
    def __init__(self, input_shapes, obs_shape, labels, device, discount=0.99, hidden_size=128, l2_loss_ratio=0.01):
        ...
        for i in range(self.n_networks):
            ...
            self.value_networks.append(net)
        self.value_next_networks = self.value_networks
    ...
    def forward(self, inputs, next_inputs, path_probs):
        ...
        value_fn = torch.mean([self.value_networks[i](inputs[i].to(torch.float32)) for i in range(self.n_networks)])
        value_fn_next = torch.mean([self.value_next_networks[i](next_inputs[i].to(torch.float32)) for i in range(self.n_networks)])
    ...
    def get_reward_weighted(self, inputs, next_inputs, path_probs, rate=[0.1, 0.1], expert_prob=True):
        with torch.no_grad():
            outputs = [self.networks[i](inputs[i].to(torch.float32)) for i in range(self.n_networks)] 
            rew_inputs = torch.cat(outputs, dim=1)
    ...
    def save(self, filename=""):
        for i in range(self.n_networks):
            ...
            fname = osp.join(logger.get_dir(), f"disc_value_{self.labels[i]}"+filename+".pth")
            torch.save(self.value_networks[i].state_dict(), fname)
    ...
    def load(self, path, filename, use_eval=False):
        for i in range(self.n_networks):
            ...
            fname = osp.join(path, f"disc_value_{self.labels[i]}"+filename+".pth")
            self.value_networks[i].load_state_dict(torch.load(fname))
    ...
    def load_with_path(self, pathes, reward_path, value_pathes, use_eval=False):
        for i in range(self.n_networks):
            ...
            self.value_networks[i].load_state_dict(torch.load(value_pathes[i]))
