...
class MultiTypeAIRL(object):
    def __init__(self, game, envs, merge_dist, conv_dist, device, experts, ppo_policies, disc_type='s_mu_a', disc_num_hidden=1, use_ppo_value=False, skip_train=[False, False, False], skip_agents=[None, None, None], common_index=[0, 1, 2], alpha_setting={"Mode":"Static", "alpha":"0.5"}):
        self._game = game
        self._envs = envs
        self._device = device
            ...
            for neps in range(num_episodes): 
                ...
                for idx, rout in enumerate(rollouts):
                    ...
                    for ob_mu in g_obs_mu[0]: 
                        ...
                        #ob_svf = np.array(list(ob_mu[:-3]) + mu_svf)
                        ob_svf = np.array(list(ob_mu[:-3]) + svf_xyt)
