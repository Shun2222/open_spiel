        ...
        while(t_step < total_step):
            ...
            for neps in range(num_episodes): 
                ...
                for idx, rout in enumerate(rollouts):
                    ...
                    update_input.append([obs_pth, logprobs_pth, actions_pth, adv_pth, returns, t_actions_pth, t_logprobs_pth])
                ...
                concated_update_input = []
                for updi in range(len(update_input[0])):
                   concated_update_input.append(torch.concat((update_input[0][updi], update_input[1][updi], update_input[2][updi]), axis=0))
                obs_pth, logprobs_pth, actions_pth, adv_pth, returns, t_actions_pth, t_logprobs_pth = concated_update_input
                v_loss = self._generator[0].update_eps(obs_pth, logprobs_pth, actions_pth, adv_pth, returns, t_actions_pth, t_logprobs_pth)
                self._generator[1]._eps_agent.load_state_dict(self._generator[0]._eps_agent.state_dict())
                self._generator[2]._eps_agent.load_state_dict(self._generator[0]._eps_agent.state_dict())
                ...
                for idx in range(self._num_agent):
                    ...
                    update_input.append([d_dxym, d_acs, d_ndxym, d_lprobs, d_labels])

                if neps%10==0:
                    concated_update_input = []
                    for updi in range(len(update_input[0])):
                       concated_update_input.append(np.concatenate((update_input[0][updi], update_input[1][updi], update_input[2][updi]), axis=0))
                    d_dxym, d_acs, d_ndxym, d_lprobs, d_labels = concated_update_input
