...
class MultiTypeAIRL(object):
    def __init__(self, game, envs, merge_dist, conv_dist, device, experts, ppo_policies):
        ...
        self._generator_mu = [MultiTypeMFGPPO(game, envs[i], merge_dist, conv_dist, device, player_id=i, expert_policy=ppo_policies[i]) for i in range(self._num_agent)]

        self._discriminator = [Discriminator(self._nobs+self._nmu, self._nacs, False, device) for _ in range(self._num_agent)]
        for i in range(self._num_agent):
            self._discriminator[i].set_player_id(i)
    ...
    def run(self, total_step, total_step_gen, num_episodes, batch_step, save_interval=1000):
        ...
        while(t_step < total_step):
            for neps in range(num_episodes): 
                ...
                for idx, rout in enumerate(rollouts):
                    ...
                    disc_rewards_pth = self._discriminator[idx].get_reward(
                        ...
                        discrim_score=False,
                        use_rewmu=False) # For competitive tasks, log(D) - log(1-D) empirically works better (discrim_score=True)
                ...
                for i in range(self._num_agent):
                    obs_pth, actions_pth, logprobs_pth, true_rewards_pth, dones_pth, values_pth, entropies_pth, t_actions_pth, t_logprobs_pth, mu_pth, ret \
                        = self._generator_mu[i].rollout(self._envs[i], batch_step)
                    rollouts.append([obs_pth, actions_pth, logprobs_pth, true_rewards_pth, dones_pth, values_pth, entropies_pth, t_actions_pth, t_logprobs_pth, mu_pth, ret])
                    mus.append(mu_pth)
                merge_mu = []
                for step in range(len(mus[0])):
                ...
                for idx, rout in enumerate(rollouts):
                    ...
                    disc_rewards_pth = self._discriminator[idx].get_reward(
                        ...
                        torch.from_numpy(logprobs).to(self._device),
                        discrim_score=False,
                        use_rewmu=True) # For competitive tasks, log(D) - log(1-D) empirically works better (discrim_score=True)
                ...
                if(num_update_eps%save_interval==0):
                    for i in range(self._num_agent):
                        ...
                        self._generator[i]_mu.save(self._game, filename=fname)
            ...
            mfg_dists = []
            for i in range(self._num_agent):
                policy = self._generator_mu[i]._ppo_policy
                mfg_dist = distribution.DistributionPolicy(self._game, policy)
                mfg_dists.append(mfg_dist)
            ...
            conv_dist = convert_distrib(self._envs, merge_dist)
            for i in range(self._num_agent):
                nashc_ppo = self._generator_mu[i].update_iter(self._game, self._envs[i], merge_dist, conv_dist, nashc=True, population=i)
                logger.record_tabular(f"nashc_ppo_mu{i}", nashc_ppo)
                nashc_expert = self._generator_mu[i].calc_nashc(self._game, merge_dist, use_expert_policy=True, population=i)
                logger.record_tabular(f"nashc_expert_mu{i}", nashc_expert)
            logger.dump_tabular()
            num_update_iter += 1
        ...
        for i in range(self._num_agent):
            ...
            fname = f"{num_update_eps}_{num_update_iter}-mu-{i}"
            self._discriminator_mu[i].save(filename=fname)
