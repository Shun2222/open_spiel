...
class BestResponse(value.ValueFunction):
  ...
  def eval_state(self, state):
    ...
    def get_entropy(state):
        logits = self._policy(state)
        probs = Categorival(logits=logits)
    ...
    if state.current_player() == pyspiel.PlayerId.MEAN_FIELD:
      ...
      reward = get_disc_rew(state.mean_field_population())
      entropy = get_entropy()
      self._state_value.set_value(
          state_str,
          reward + entropy + max_q)
      return self._state_value(state_str)
  ...
  def value(self, state, action=None):
    ...
    new_state = state.child(action)
    reward = get_disc_rew(state.mean_field_population())
    return reward + entropy + self._state_value(
