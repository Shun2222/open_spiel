...
def parse_args():
    ...
    parser.add_argument("--logdir", type=str, default="/mnt/shunsuke/result/1209/4rooms_maze_single_airl_in_multi_15trajs_dxdy_svf2_updper10", help="log path")
...
if __name__ == "__main__":
    args = parse_args()
    if args.share:
        assert False, "Not exist share mode!"
    if not args.dxy_input:
        from open_spiel.python.mfg.algorithms.multi_type_adversarial_inverse_rl import MultiTypeAIRL
        assert False, "Not adapt state input!"
    else:
        from open_spiel.python.mfg.algorithms.multi_type_adversarial_inverse_rl_in_multi_dxy import MultiTypeAIRL
